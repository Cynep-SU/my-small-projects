    ...
    def can_move(self, board, row, col, row1, col1):
        ...
        if row != row1 and col != col1:
            if row1 - row == col1 - col:
                step = 1 if (col1 >= col) else -1
                if step == -1:
                    for i in range(-1, col1 - col, step):
                        if not (board.get_piece(row + i - 1, col + i - 1) is None):
                            if not (board.get_piece(row1, col1) is None) and row1 == row + i - 1:
                                return not board.get_piece(row1, col1).get_color() == self.color
                            return False
                    return True
                for i in range(1, col1 - col, step):
                    if not (board.get_piece(row + i, col + i) is None):
                        if not (board.get_piece(row1, col1) is None) and row1 == row + i:
                            return not board.get_piece(row1, col1).get_color() == self.color
                        return False
                return True
            elif row1 - row == col - col1:
                step = 1 if (col1 >= col) else -1
                if step == -1:
                    for i in range(-1, col1 - col, step):
                        if not (board.get_piece(-(row + i - 1), col + i - 1) is None):
                            ...
                            return False
                    try:
                        print(-(row + i - 1), col + i - 1, row1, col1)
                    except:
                        return True
                for i in range(1, col1 - col, step):
                    if not (board.get_piece(row + i, col + i) is None):
